...
A complete feature list of ObiWan-Microbi

- **SegServe - Remote AI execution**: Execute AI approaches on a single server with specialized hardware (e.g. GPU).
  - **Expandable AI Zoo**: Segmentation approaches are packaged as git repositories using [mflow](https://mlflow.org/) and [anaconda](https://www.anaconda.com/) for dependency management. Installations are performed on demand. Your custom model only needs a git repo!
...
- **User-friendly ground truth annotation (SegUI)**:
  - **Platform independent**: WebApp based on [Angular](https://angular.io/) and [Ionic](https://ionicframework.com/) frameworks to run in the browser on any browser capable device (e.g. notebook, tablet, smartphone) and no installation is needed.
  ...
  - **Manual correction tools**: Providing brush tool for drawing segmentations and a multi-select tool for selecting and deleting multiple segmentation contours. Correcting erroneous AI segmentation or starting from scratch becomes convenient.
  - **Smart data model**: Undo/redo segmentation changes to flexibly try all features without fearing mistakes or progress loss.
  ...
  - **Automated Saving**: Do not loose any progress. We automatically save your work in the background (every 5 seconds).
- **OMERO Integration**: User and data management using [Omero](https://www.openmicroscopy.org/omero/) allows to collaborate and scale to large teams.
...
- **Facilitate collaboration**: OMERO allows to share data in groups such that ground truth creation can be performed in a team of experts. All team members have access to the most recent progress.
